...
class Title:
    ...
    def draw(self, screen):
        ...
        screen.blit(prompt_text, (130, 250))
...
class Menu:
    ...
    def draw(self, screen):
        ...
        # Приветствие
        welcome = self.small_font.render(f"Hello, {self.game.current_username}!", True, (0, 255, 255))
        screen.blit(welcome, (20, 20))

        # Отрисовка кнопок
        btns = [
            (self.btn_play, "PLAY", (0, 255, 0)),
            (self.btn_leader, "LEADERS", (255, 215, 0)),
            (self.btn_settings, "SETTINGS", (255, 255, 0)),
            (self.btn_quit, "QUIT", (255, 0, 0))
        ]
...
class Leaderboard:
    ...
    def draw(self, screen):
        ...
        # Заголовок
        title = self.font_title.render("TOP 10 LEADERS", True, (255, 215, 0))
        screen.blit(title, (600 // 2 - title.get_width() // 2, 30))

        # Шапка таблицы
        header = self.font_text.render("RANK  NAME      SCORE  LVL   DATE", True, (150, 150, 150))
        ...
        for i, row in enumerate(self.top_data):
            try:
                ...
                color = (0, 255, 255) if name_val == self.game.current_username else (255, 255, 255)
        ...
        # Кнопка назад
        pygame.draw.rect(screen, (70, 70, 70), self.btn_back, border_radius=10)
...
class Playing:
    ...
    def draw(self, screen):
        ...
        for block in self.obstacles.blocks:
            pygame.draw.rect(screen, (100, 100, 100), (*block, 20, 20))
            pygame.draw.rect(screen, (150, 150, 150), (*block, 20, 20), 2)

        # 3. Отрисовка еды
        if self.game.food.is_poison:
            food_color = (0, 0, 255)  # Синий яд
        else:
        ...
        snake_color = self.game.settings.data.get("snake_color", [0, 200, 0])
...
class GameOver:
    def __init__(self, game):
        self.game = game
        self.font_title = pygame.font.SysFont('Arial', 45)
    ...
    def draw(self, screen):
        screen.fill((40, 10, 10)) # Темно-красный фон для экрана проигрыша

        # 1. Заголовок
        title_surf = self.font_title.render('GAME OVER', True, (255, 0, 0))
        screen.blit(title_surf, (600 // 2 - title_surf.get_width() // 2, 50))
        ...
        for i, text in enumerate(stats):
            color = (255, 255, 255)
            if "Score" in text: color = (0, 255, 0) # Подсветим счет зеленым
            stat_surf = self.font_stats.render(text, True, color)
            screen.blit(stat_surf, (600 // 2 - stat_surf.get_width() // 2, 150 + i * 40))

        # 3. Отрисовка кнопок
        # Кнопка Retry
        pygame.draw.rect(screen, (50, 50, 50), self.btn_retry, border_radius=10)
        pygame.draw.rect(screen, (100, 100, 100), self.btn_retry, 2, border_radius=10)
        ...
        # Кнопка Main Menu
        pygame.draw.rect(screen, (50, 50, 50), self.btn_menu, border_radius=10)
        pygame.draw.rect(screen, (100, 100, 100), self.btn_menu, 2, border_radius=10)
        ...
        # 4. Подсказка внизу
        hint = self.font_stats.render("Use mouse or SPACE to restart", True, (150, 150, 150))
...
class SettingsMenu:
    def __init__(self, game):
        ...
        self.colors = [("Green", [0, 200, 0]), ("Blue", [0, 0, 255]), ("Red", [255, 0, 0])]
        self.c_idx = 0
        
        # Синхронизация индекса с текущими настройками
        current_color = self.game.settings.data.get("snake_color", [0, 200, 0])
        ...
        self.btn_color = pygame.Rect(200, 150, 250, 40)
        self.btn_grid = pygame.Rect(200, 210, 250, 40)
        self.btn_sound = pygame.Rect(200, 270, 250, 40)
        self.btn_save = pygame.Rect(200, 380, 250, 50)
    ...
    def draw(self, screen):
        ...
        # Заголовок
        title = self.title_font.render("SETTINGS", True, (255, 215, 0))
        ...
        pygame.draw.rect(screen, (0, 100, 0), self.btn_save, border_radius=10)
